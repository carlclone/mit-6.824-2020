	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				if ok {
					DPrintf("leader %v start 成功 , cmd  , index %v", starts, index1)
					index = index1
					break
				}
			}
		...
		//是 leader 的话会 append 到 log 里并且等待一段时间后期望能看到其他 peer 都已经复制
		if index != -1 {
